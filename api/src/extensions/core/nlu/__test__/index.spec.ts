...
import NluHelper from '../index.helper';

describe('NLP Default Helper', () => {
  let settingService: SettingService;
  let nlpService: NlpService;
  let defaultNlpHelper: NluHelper;
  ...
  beforeAll(async () => {
    ...
    defaultNlpHelper = module.get<NluHelper>(NluHelper);
  ...
  it('should return best guess from empty parse results', async () => {
    const nlp = nlpService.getNLP();
    const results = await nlp.bestGuess(
      {
        entities: [],
        intent: {},
        intent_ranking: [],
  ...
  it('should return best guess from parse results', async () => {
    const nlp = nlpService.getNLP();
    const results = await nlp.bestGuess(nlpParseResult, false);
    expect(results).toEqual(nlpBestGuess);
  });

  it('should return best guess from parse results with threshold', async () => {
    const nlp = nlpService.getNLP();
    const results = await nlp.bestGuess(nlpParseResult, true);
    const settings = await settingService.getSettings();
    const threshold = settings.nlu.threshold;
